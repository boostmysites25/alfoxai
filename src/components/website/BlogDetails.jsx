...
const BlogDetails = ({ blog, onBack }) => {
  ...
  const relatedBlogs = blog.relatedPosts 
    ? blog.relatedPosts.map(id => blogPosts.find(post => post.id === id)).filter(Boolean)
    : [];
      ...
      <article>
        <div className="mb-8">
          ...
          <div className="flex flex-wrap items-center text-tertiary mb-6 gap-4">
            ...
            <span>By <span className="text-highlight">{blog.author}</span> {blog.authorRole && <span className="text-tertiary">({blog.authorRole})</span>}</span>
            <span className="hidden md:inline">•</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {relatedBlogs.map(relatedBlog => (
                ...
                >
                  ...
                  <div className="p-4">
                    <h4 className="text-lg font-bold mb-2 text-white line-clamp-2">{relatedBlog.title}</h4>
                    <p className="text-tertiary text-sm line-clamp-2">{relatedBlog.excerpt}</p>
                  </div>
                </div>
              ))}
            </div>
